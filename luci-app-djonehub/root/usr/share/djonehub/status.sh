...
VERSION_FILE="/etc/djonehub/bin/version"
ARCH_FILE="/etc/djonehub/bin/arch"
BACKUP_VERSION_FILE="/etc/djonehub/bin/version.bak"
METRICS_STATE="/tmp/djonehub/status.metrics"
...
uci_get() {
	...
	value="$(uci -q get "djonehub.main.$key" 2>/dev/null || true)"
...
djonehub_pids() {
	...
	for pid in /proc/[0-9]*; do
		...
		case "$cmd" in
			*"/etc/djonehub/bin/djonehub"*) printf '%s\n' "$pid" ;;
		esac
	done
}
...
is_running=0
/etc/init.d/djonehub running >/dev/null 2>&1 && is_running=1
