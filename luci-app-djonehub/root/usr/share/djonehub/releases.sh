...
json="$(curl -fsSL --show-error --connect-timeout 8 --max-time 25 "https://api.github.com/repos/$repo/releases?per_page=$limit" 2>/tmp/djonehub-releases.err)" || {
	msg="$(cat /tmp/djonehub-releases.err 2>/dev/null || true)"
