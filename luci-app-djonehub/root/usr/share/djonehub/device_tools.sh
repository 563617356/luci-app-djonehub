...
task_enabled() {
	[ -n "${DJONEHUB_TASK_ID:-}" ] && [ -n "${DJONEHUB_TASK_TYPE:-}" ] && [ -f /usr/share/djonehub/task_lib.sh ]
}
...
task_progress() {
	...
	if task_enabled; then
		# shellcheck source=/usr/share/djonehub/task_lib.sh
		. /usr/share/djonehub/task_lib.sh
		task_log "$DJONEHUB_TASK_ID" "$message"
		task_write_status "$DJONEHUB_TASK_ID" "$DJONEHUB_TASK_TYPE" "running" "$stage" "$message" "" 0 0 0 0
	fi
}
...
at_with_socat() {
	...
	local tmp="/tmp/djonehub/socat.$$"
	local input="/tmp/djonehub/socat-in.$$"
...
stop_djonehub() {
	task_progress service "停止 DJOneHub 服务"
	/etc/init.d/djonehub stop 2>&1 || true
}

start_djonehub() {
	task_progress service "启动 DJOneHub 服务"
	/etc/init.d/djonehub start 2>&1 || true
}
...
convert_json() {
	...
	output="停止 DJOneHub 服务...\n"
	output="$output$(stop_djonehub)\n\n"
	...
	output="${output}启动 DJOneHub 服务...\n"
	output="$output$(start_djonehub)\n"
	...
	case "$ok" in
		...
		partial)
			...
			;;
		*)
			printf '{"ok":false,"message":"写入命令未确认成功，DJOneHub 已启动","output":"%s"}\n' "$(json_escape "$output")"
			exit 1
			;;
	esac
}
